...
import { getMessages, ChatSourceError, type Platform } from '@/lib/chat-source/client';
...
export async function GET(req: Request) {
  ...
  if (!shopId || !convId) return NextResponse.json({ error: 'shop_id และ conversation_id required' }, { status: 400 });
  const platform: Platform = searchParams.get('platform') === 'tiktok' ? 'tiktok' : 'shopee';

  try {
    const data = await getMessages(platform, shopId, convId, {
