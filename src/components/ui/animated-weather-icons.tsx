      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-8 justify-items-center">
        {ALL_ICONS.map(({ name, Icon }) => (
          <div key={name} className="flex flex-col items-center gap-3">
            <div className="flex items-center justify-center size-20 rounded-2xl border border-slate-200 bg-white shadow-card">
              <Icon size={48} />
            </div>
            <span className="text-[11px] font-medium text-slate-500 tracking-wide text-center leading-tight">
              {name}
            </span>
          </div>
        ))}
